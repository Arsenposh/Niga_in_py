...
BEGIN
    ...
    IF p_type NOT IN ('home', 'work', 'mobile') THEN
        RAISE EXCEPTION 'Неверный тип телефона: %. Допустимые: home, work, mobile', p_type;
    END IF;
    IF EXISTS (
        SELECT 1 FROM phones
        WHERE contact_id = v_contact_id AND phone = p_phone
    ) THEN
        RAISE NOTICE 'Номер % уже существует у контакта "%"', p_phone, p_contact_name;
        RETURN;
    END IF;
    INSERT INTO phones (contact_id, phone, type)
    VALUES (v_contact_id, p_phone, p_type);

    RAISE NOTICE 'Номер % (%) добавлен контакту "%"', p_phone, p_type, p_contact_name;
END;
$$;
